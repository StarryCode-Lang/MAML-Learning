...
def main(args):
    ...
    for step in range(args.epoch):
        ...
        if step % log_interval == 0:
            ...
            print(
                ...
                step,
                " [",
                " ".join([f"{x:.4f}" for x in accs]),
                "]",
                f"\tavg_time/step(s): {round(avg_step_time, 4)}",
                f"\tthroughput(img/s): {round(images/avg_step_time, 1)}",
            )
            tot_elapsed = 0.0
            steps_since_log = 0
        ...
        if step % 1000 == 0:  # 增加评估间隔，从500增加到1000
            ...
            if current_acc > best_test_acc:
                ...
                if not hasattr(maml, "best_acc") or current_acc > maml.best_acc:
                    ...
                    saved_checkpoints.append(
                        {"path": save_path, "step": step, "accuracy": current_acc}
                    )
            ...
            print(
                ...
                " ".join([f"{x:.4f}" for x in accs]),
                "]",
                f"\teval_time(s): {round(eval_elapsed, 2)}",
            )
    ...
    final_acc, final_accs, final_eval_elapsed, final_save_path = evaluate_model(
        ...
        device,
        args,
        sample_num=200 // args.task_num,
        step=args.epoch - 1,
    )
    print(
        "最终评估 acc:",
        " ".join([f"{x:.4f}" for x in final_accs]),
        f"\teval_time(s): {round(final_eval_elapsed, 2)}",
    )

    saved_checkpoints.append(
        {"path": final_save_path, "step": args.epoch - 1, "accuracy": final_acc}
    )
    ...
    if saved_checkpoints:
        ...
        final_best_path = (
            f'checkpoints/maml_best_final_acc{best_checkpoint["accuracy"]:.4f}.pth'
        )
        ...
        print(f"✓ 训练完成！")
        print(
            f'✓ 最佳模型: step={best_checkpoint["step"]}, acc={best_checkpoint["accuracy"]:.4f}'
        )
